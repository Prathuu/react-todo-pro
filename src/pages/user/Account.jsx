...
export default function Account() {
  ...
  const getAllTodos = async () => {
    const { data } = await axios
      .get("https://todo-pro-backend.herokuapp.com/todos")
    console.log(data);
    settodos(data)
    settodo(data)
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const { data } = await axios.put(`https://todo-pro-backend.herokuapp.com/todos/${editId}`,
      {
  ...
  const handleDelete = async (id) => {
    const { data } = await axios.delete(`https://todo-pro-backend.herokuapp.com/todos/${id}`)
    getAllTodos()
  }
